        ...
        <>
            ...
            {isMobile && (
                <button 
                    ...
                    className="fixed z-50 top-4 left-4 p-3 rounded-xl bg-gradient-to-r from-blue-800 to-blue-900 text-white shadow-lg hover:shadow-blue-500/25 hover:scale-105 transform transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50 lg:hidden"
            ...
            <aside 
                ...
                bg-gradient-to-b from-slate-900 via-blue-900 to-blue-950 text-white shadow-2xl border-r border-blue-700/50 backdrop-blur-sm`}
            >
                <div className="flex flex-col h-full">
                    {/* Header with Logo */}
                    <div className={`flex items-center justify-between px-6 py-5 border-b border-blue-700/50 ${collapsed && !isMobile ? 'justify-center px-4' : ''}`}>
                        <div className="flex items-center space-x-3">
                            <div className="flex-shrink-0 bg-gradient-to-br from-blue-600 to-blue-800 p-2.5 rounded-xl shadow-lg transform hover:scale-110 transition-transform duration-300 group">
                            ...
                            {(!collapsed || isMobile) && (
                                <h2 className="text-xl font-bold text-white tracking-wider animate-fadeIn drop-shadow-md">
                                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-blue-300">Insurance</span>
                        ...
                        {!isMobile && (
                            <button 
                                onClick={() => setCollapsed(!collapsed)}
                                className="text-gray-400 hover:text-white hover:bg-blue-700/50 p-2 rounded-lg transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50"
                        ...
                        {navigationItems.map((item, itemIndex) => {
                            ...
                            return (
                                <Link 
                                    ...
                                    ${active 
                                        ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg shadow-blue-900/25 transform scale-105' 
                                        : 'text-gray-300 hover:bg-blue-700/50 hover:text-white hover:transform hover:scale-105'
                                    }`}
                                >
                    ...
                    <div className={`mt-auto p-5 border-t border-blue-700/50 ${collapsed && !isMobile ? 'flex flex-col items-center' : ''}`}>
                        <div className={`${collapsed && !isMobile ? 'text-center' : 'flex items-center mb-4'}`}>
                            <div className={`relative w-12 h-12 rounded-full bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center text-white font-bold text-lg
                                shadow-lg hover:shadow-blue-500/25 transform hover:scale-110 transition-all duration-300
                                ${collapsed && !isMobile ? 'mx-auto mb-3' : 'mr-3'}`}>
                                {getInitials()}
                                <span className="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-blue-400 border-2 border-slate-800 rounded-full animate-pulse"></span>
                            </div>
                            {(!collapsed || isMobile) && (
                                <div className="animate-fadeIn overflow-hidden">
                                    ...
                                    <p className="text-xs text-blue-300 tracking-wide truncate">
                                        {user.email || `${role}@insurancehub.com`}
                                    </p>
                                </div>
                            )}
                        </div>
